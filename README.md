...
2. Run `./docker/run-fdo-owner-service.sh -h` to see the usage, and set all the necessary environment variables. For example:
   ...
   export FIDO_DEVICE_ONBOARD_REL_VER=1.1.10 # https://github.com/fido-device-onboard/release-fidoiot/releases
...
- If the edge device does not give a `[INFO ] TO2 completed successfully. [INFO ] Starting Fdo Completed`, check /fdo/pri-fidoiot-v1.1.10/owner/app-data/service.log or use command `docker logs -f fdo-owner-service` for error messages.
