...
Recommended environment variables:
  FDO_OWNER_SVC_HOST - external hostname or IP that the RV should tell the device to reach OPS at. Defaults to the host's hostname but that is only sufficient if it is resolvable and externally accessible.
Additional environment variables (that do not usually need to be set):
  ...
  FDO_OCS_API_PORT - port number OCS-API should listen on for HTTP. Default is 9008.
  FDO_OCS_API_TLS_PORT - port number OCS-API should listen on for TLS. Default is the value of FDO_OCS_API_PORT. (OCS API does not support TLS and non-TLS simultaneously.) Note: you can not set this to 9009, because OCS listens on that port internally.
  FDO_API_CERT_HOST_PATH - path on this host of the directory holding the certificate and key files named sdoapi.crt and sdoapi.key, respectively. Default is for the OCS-API to not support TLS.
  FDO_API_CERT_PATH - path that the directory holding the certificate and key files is mounted to within the container. Default is /home/sdouser/ocs-api-dir/keys .
  EXCHANGE_INTERNAL_URL - how OCS-API should contact the exchange for authentication. Will default to HZN_EXCHANGE_URL.
  EXCHANGE_INTERNAL_CERT - the base64 encoded certificate that OCS-API should use when contacting the exchange for authentication. Will default to the sdoapi.crt file in the directory specified by FDO_API_CERT_HOST_PATH.
...
if [[ -z "$FDO_API_PWD" || -z "$HZN_EXCHANGE_URL" || -z "$HZN_FSS_CSSURL" || -z "$HZN_MGMT_HUB_CERT" ]]; then
    echo "Error: These environment variable must be set to access Owner services APIs: FDO_API_PWD, HZN_EXCHANGE_URL, HZN_FSS_CSSURL, HZN_MGMT_HUB_CERT"
    exit 0
fi
...
export FDO_OCS_API_TLS_PORT=${FDO_OCS_API_TLS_PORT:-$FDO_OCS_API_PORT}
...
export FDO_OWNER_EXTERNAL_PORT=${FDO_OWNER_EXTERNAL_PORT:-$FDO_OWNER_PORT}   # the external port the device should use to contact OPS
export dbPort=${FDO_DB_PORT:-5432}
# Define the OPS hostname the to0scheduler tells RV to direct the booting device to
FDO_OWNER_SVC_HOST=${FDO_OWNER_SVC_HOST:-$(hostname)}   # currently only used for OPS
HZN_FDO_SVC_URL="http://"$FDO_OWNER_SVC_HOST":"$FDO_OPS_PORT
...
portNum=${FDO_OCS_API_TLS_PORT:-$FDO_OCS_API_PORT}
